...
export default function InstitutionInvitePage() {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`/api/institution-invites/${token}`).then((r) => (r.ok ? r.json() : null)),
  ...
  async function accept() {
    if (!loggedInEmail) {
      router.push(`/auth/login?next=/institution-invites/${token}`);
      return;
    }
    setStatus("accepting");
    ...
    if (res.ok) {
      setStatus("done");
      notify.success("You've joined the institution");
      setTimeout(() => router.push(`/dashboard/institutions/${data.institutionId}`), 1600);
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <main className="flex-1 flex items-center justify-center px-4">
        ...
        >
          {status === "loading" && (
            <div className="card p-10 text-center text-[color:var(--muted)] text-sm">
              Checking invite…
            </div>
          )}
          ...
          {status === "ready" && info && (
            <div className="card p-8">
              <div className="flex items-center gap-3 mb-5">
                ...
                <div>
                  <h1 className="text-xl font-extrabold">Institution invite</h1>
                  <p className="text-xs text-[color:var(--muted)]">
                    You&apos;ve been invited to join an institution
                  </p>
                </div>
              </div>
              ...
              <div className="p-4 bg-indigo-50 border border-indigo-100 rounded-xl mb-5 space-y-1 text-sm">
                <div>
                  <span className="text-[color:var(--muted)]">Institution: </span>
                  <strong>{info.institutionName}</strong>
                </div>
                <div>
                  <span className="text-[color:var(--muted)]">Invited email: </span>
                  <strong>{info.inviteeEmail}</strong>
                </div>
              </div>
              ...
              {loggedInEmail ? (
                <>
                  <p className="text-sm text-[color:var(--muted)] mb-4">
                    ...
                    {loggedInEmail.toLowerCase() !== info.inviteeEmail.toLowerCase() && (
                      <span className="text-amber-700">
                        ⚠ This invite was sent to a different email. Please sign in with the correct
                        account.
                      </span>
                    )}
                  </p>
                  ...
                  {loggedInEmail.toLowerCase() !== info.inviteeEmail.toLowerCase() && (
                    <Link
                      href={`/auth/login?next=/institution-invites/${token}`}
                ...
                <>
                  <p className="text-sm text-[color:var(--muted)] mb-4">
                    You need a Neurogauge account to accept this invite. If you don&apos;t have one,
                    create one with the invited email address (<strong>{info.inviteeEmail}</strong>).
                  </p>
                  <Link
                    href={`/auth/login?next=/institution-invites/${token}`}
                  ...
                  <Link
                    href={`/auth/signup?next=/institution-invites/${token}`}
                    className="btn btn-ghost w-full block text-center mt-2"
                  >
                    Create account
                  </Link>
                </>
          ...
          {status === "done" && (
            <div className="card p-8 text-center">
              ...
              <p className="text-sm text-[color:var(--muted)] mt-2">
                You&apos;ve joined <strong>{info?.institutionName}</strong>. Redirecting…
              </p>
            </div>
          )}
          ...
          {(status === "error" || status === "wrongEmail") && (
            <div className="card p-8 text-center">
              ...
              <h1 className="text-xl font-extrabold">
                {status === "wrongEmail" ? "Wrong account" : "Error"}
              </h1>
              <p className="text-sm text-[color:var(--muted)] mt-2">{errorMsg}</p>
              <button onClick={() => setStatus("ready")} className="btn btn-ghost mt-4">
                Go back
              </button>
            </div>
          )}
