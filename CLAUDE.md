...
- **AI Statistician** (AI-assisted analysis, first vertical slice built; requires `OPENAI_API_KEY`): shared AI client (`src/lib/ai/`) drives two workbench features.
  ...
  - ⚠️ **Still planned (rest of the AI-stats feature set, not yet built)**: **AI participant randomization**, **APA tables & figures** export, and **Results-section drafting**. These will extend the same `src/lib/ai/` client + `/api/ai/*` route pattern.
