...
- **Account types & subscriptions**: Every user has an `AccountType` (`AccountType` enum: `student` / `institution` / `research_group`) chosen at signup (`src/app/auth/signup/SignupClient.tsx` — a 3-card chooser; defaults to `student`). The account type is stored on `User.accountType` and **drives**: (a) a **type-specific dashboard** (`/dashboard` branches on `accountType`: students see just their project grid; institution/research_group accounts additionally get a `SupervisorSummary` rollup + a **Sites** link on top, via shared components in `src/components/dashboard/`); and (b) the **per-type subscription plans**. Projects belong directly to a user (`Project.ownerId`); sharing is per-project via `ProjectCollaborator`. There is a lightweight **Institution** layer for multicenter collaboration (see **Collaborating institutions** below) — a first-class org whose members sign in and see co-worked projects scoped to their own site — but projects are still owned by a single user, not by an institution.
...
- **Collaborating institutions** (multicenter cross-org collaboration): a first-class **Institution** layer so partner centres get their own sign-in + dashboard. Models (`prisma/migrations/20260907120000_add_institutions`): `Institution` (name + globally-unique `code`), `InstitutionMember` (`InstitutionRole` enum `owner|admin|member`; a user↔institution membership = the sign-in ability), `InstitutionInvite` (email invite to join, mirrors `CollaboratorInvite`), and `ProjectInstitution` (a project lead links an institution to a project and assigns it the collection **`Site`** — `@@unique([projectId, institutionId])`). Helpers in `src/lib/institution.ts` (`slugifyInstitutionCode`, `getMembership`, `canManage`).
  - **Data scope is own-site-only**: an institution member's dashboard (`/dashboard/institutions/[id]`, data from `GET /api/institutions/[id]`) shows the projects their institution co-works on, but each project's stats/collect-link are scoped to **that institution's assigned site** — sessions where `siteId === ProjectInstitution.siteId`. They do **not** see other sites' data or full project analytics.
  - **Flows**: create/list institutions (`/dashboard/institutions`, `GET/POST /api/institutions` — creator becomes `owner`); invite members (`POST /api/institutions/[id]/members`, accept at `/institution-invites/[token]` via `GET/POST /api/institution-invites/[token]` — mirrors the project-invite pages/routes; `institutionInviteEmail` in `src/lib/mail.ts`); the project lead links institutions from the project **Collaborators** tab (`ProjectInstitutions` component → `GET/POST/DELETE /api/projects/[id]/institutions`, owner-only, site validated against the owner's sites). Institution members collect at their site using the existing `/p/[shareToken]?site=<code>` link surfaced on their dashboard.
...
- `/api/projects/[id]/institutions` - GET/POST/DELETE (owner-only): list / link (by institution `code` + assign a `siteId` validated against the owner's sites) / unlink collaborating institutions
- `/api/institutions` - GET (institutions the caller is a member of + counts) / POST (create; creator becomes `owner`)
- `/api/institutions/[id]` - GET (member-scoped institution dashboard: members, pending invites for managers, co-worked projects with **own-site-only** participant/completion stats) / DELETE (owner-only)
- `/api/institutions/[id]/members` - POST (owner/admin invite by email → `InstitutionInvite` + email) / DELETE `?memberId=` (managers remove others; anyone can self-leave; owner can't be removed)
- `/api/institution-invites/[token]` - GET (public invite info) / POST (accept — adds the signed-in user as a member with the invited role)
